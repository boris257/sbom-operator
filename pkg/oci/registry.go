...
func ConvertSecrets(img RegistryImage, proxyRegistryMap map[string]string) []RegistryCredentials {
	credentials := make([]RegistryCredentials, 0)
	for _, secret := range img.PullSecrets {
		...
		credentials = append(credentials, RegistryCredentials{
